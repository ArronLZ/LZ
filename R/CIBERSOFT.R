...
#' CIBERSORT
#' @description run cibersort
#' 
#' @param sig_matrix matrix, signature matrix, LM22
#' @param mixture_obj matrix, mixture matrix, row is gene, column is sample
#' @param perm number, default 1000(recommend 1000)
#' @param QN logical, default F, RNAseq data: recommend QN = F, otherwise T
#' @param absolute absolute logical, if T, return absolute score
...
#' @examples
#' \dontrun{
#'  CIBERSORT(sig_matrix_file = LM22, 
#'            mixture_obj = exprs, 
#'            perm = 1000, QN = F, absolute = T, abs_method='sig.score')
#' }
CIBERSORT <- function(sig_matrix, mixture_obj, perm=0, QN=TRUE, 
  ...
  #read in data
  # X <- read.table(sig_matrix_file, header=T, sep="\t", row.names=1, check.names=F) #j
  X <- sig_matrix
  Y <- mixture_obj #j
